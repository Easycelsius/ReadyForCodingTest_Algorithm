...
public static int solution(int[] arrows) {
        ...
        for(int arrow:arrows){
            for(int i=1; i<=1; i++){ // 교차점 처리를 위한 스케일업(반복 2번)
                
                System.out.println(arrow +" / " + point[0]+" , "+point[1]);
                ...
                } else if(containsKeyInMap(visitied, newPoint) && !(containsInList(visitied.get(newPoint), point))){ 
                    visitied.get(newPoint).add(point);
                    visitied.get(point).add(newPoint);
                    cnt++;
                }
