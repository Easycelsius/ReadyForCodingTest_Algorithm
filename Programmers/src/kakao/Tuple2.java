...
public class Tuple2 {
	...
	public static int[] solution(String s) {
		...
		ArrayList<Integer> smallListLen = new ArrayList<>();
		HashMap<Integer, ArrayList> hashMap = new HashMap<>(); // 길이, 어레이리스트
		...
		for (String a : s.split("\\},\\{")) {
			...
			smallListLen.add(smallList.size());
			smallList.clear();
		}

		// 해쉬 맵 구성
		for (int i = 0; i < bigList.size(); i++) {
			hashMap.put(smallListLen.get(i), bigList.get(i));
		}
		...
		// 정답 넣기 smallList 재활용
		for (ArrayList al : hashMap.values()) {
		...
		// 배열화
		int[] answer = new int[bigList.size()];
